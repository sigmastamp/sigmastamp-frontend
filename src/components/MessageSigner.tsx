...
export function MessageSigner(props: {
	...
	const [loadedAddresses, setLoadedAddresses] = useState<IAddressesLoader>(DEFAULT_ADDRESSES_LOADER_STATE);
	const [userIsOwner, setUserIsOwner] = useState<IUserIsOwnerOfAddress>(DEFAULT_USER_IS_OWNER_OF_ADDRESS_STATE);
	...
	useEffect(() => {
		...
		ergoConnector.nautilus.getContext().then((ctx) => {
			ctx.get_used_addresses().then((addresses) => {
				...
				if(addresses.indexOf(props.stamperAddress) > -1) intersectingAddresses.push(props.stamperAddress);
				if(addresses.indexOf(props.holderAddress) > -1) intersectingAddresses.push(props.holderAddress);
				if(intersectingAddresses.length == 2 && intersectingAddresses[0] === intersectingAddresses[1]) intersectingAddresses.pop();
				setUserIsOwner({
					ownerOfStamperAddress: (addresses.indexOf(props.stamperAddress) > -1),
					ownerOfHolderAddress: (addresses.indexOf(props.holderAddress) > -1),
		...
		<div>
			{
				(!props.wallet.connected) ?
					<h3>Are you the original "stamper" or current holder of NFT token and want to sign a message? Please connect a wallet first.</h3>
					:
				(!loadedAddresses.loaded) ? <h3>Loading your addresses, wait a bit...</h3> :
